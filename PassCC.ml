  ...
  match !r with
    ...
    | LetFun (f, args, k, body, e) ->
      ...
      r := LetPack (id_pack, S.elements esc, ref (
            LetFun (f, args @ [id_envp], k, body, ref (
    ...
    | LetRec (bs, e) ->
      ...
      let (id, esc) = List.fold_left (fun (id, total) (_, args, _, body) ->
        let (id, esc) = cc body id in
        let esc = List.fold_left (fun esc a -> S.remove a esc) esc args in
      ...
      let esc = List.fold_left (fun esc (f, _, _, _) -> S.remove f esc) esc bs in

      let id_pack, id = id, id + 1 in
      let id_envp, id = id, id + 1 in

      let bs = List.map (fun (f, args, k, body) ->
        let (new_body, slot) = List.fold_left (fun (body, slot) (f, _, _, _) ->
          (ref (LetProj (f, slot, id_envp,
            ref (LetPack (f, [f; id_envp], body)))), slot + 1)) (body, 0) bs in
        let _ = S.fold (fun esc slot ->
          body := LetProj (esc, slot, id_envp, ref (!body));
          slot + 1) esc slot in
        (f, args @ [id_envp], k, new_body)) bs in

      let fat_env = esc
        |> S.elements
        |> List.fold_right (fun (f, _, _, _) xs -> f :: xs) bs in
      let tail = List.fold_left (fun e (f, _, _, _) ->
        ref (LetPack (f, [f; id_pack], e))) e bs in
      r := List.fold_right (fun (f, args, k, body) tail ->
        LetFun (f, args, k, body, ref tail)) bs (LetPack (id_pack, fat_env, tail));

      let (id, fv2) = cc e id in
      let fv2 = List.fold_left (fun fv2 (f, _, _, _) -> S.remove f fv2) fv2 bs in
      (id, S.union esc fv2)
